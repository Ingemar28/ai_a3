...
while True:
    ...
    if is_done:
        ...
        if len(all_rewards) > 10 and len(losses) > 10:
            ...
            if not visualizer_on and r100 > 0.99 * params['stopping_reward']:
...
r100_values = [np.mean(all_rewards[i:i + 100]) for i in range(len(all_rewards) - 99)]

plt.plot(range(1, len(all_rewards) + 1), all_rewards, label='Reward')
plt.plot(range(1, len(r100_values) + 1), r100_values, label='R100')
plt.xlabel("Episode")
plt.ylabel("R100")
plt.title("Reward and R100 vs Episode")
plt.grid(True)
plt.legend()
plt.show()